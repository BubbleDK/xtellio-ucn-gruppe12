    ...
    <div class="grid grid-cols-4 gap-4 mx-auto max-w-7xl py-6">
      <div class="bg-gray-800 p-6 rounded-lg shadow-lg" v-for="device in customerDevices">
        <div class="flex justify-between items-baseline pb-2 ">
          <div class="flex ">
            <span v-if="device.state == 'Active'"
              class="bg-teal-200 text-black text-xs px-2 inline-block rounded-full  uppercase font-semibold tracking-wide">
              {{ device.state }}
            </span>
            <span v-else-if="device.state == 'Inactive'"
              class="bg-red-200 text-black text-xs px-2 inline-block rounded-full  uppercase font-semibold tracking-wide">
              {{ device.state }}
            </span>
            <span v-else-if="device.state == 'Factory'"
              class="bg-yellow-200 text-black text-xs px-2 inline-block rounded-full  uppercase font-semibold tracking-wide">
              {{ device.state }}
            </span>
            <span v-else
              class="bg-gray-400 text-black text-xs px-2 inline-block rounded-full  uppercase font-semibold tracking-wide">
              {{ device.state }}
            </span>
          </div>
          <div>
            <span class="text-sm font-medium text-gray-500 dark:text-gray-400">QR: </span>
            <span v-if="device?.ids?.qr !== null" class="text-sm font-medium text-gray-500 dark:text-gray-400">{{
              device?.ids?.qr }}</span>
